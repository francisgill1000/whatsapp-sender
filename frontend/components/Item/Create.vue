  ...
  <div class="text-center">
    <v-dialog v-model="dialog" width="400">
      ...
      <template v-slot:activator="{ on, attrs }">
        <v-btn class="white primary--text" rounded x-small v-bind="attrs" v-on="on">
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="12" class="text-right">
              ...
              &nbsp;
              <v-btn :loading="loading" @click="submit" class="primary" small
                >Submit</v-btn
              >
            </v-col>
          </v-row>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      try {
        await this.$axios.post("/items", formData, {
          headers: {
